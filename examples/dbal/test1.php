...
\React\EventLoop\Loop::addPeriodicTimer(5, function () use($executor, $dbConnectionString) {
    print "Going to create a new user\n";
    $executor->execute(function() use($dbConnectionString) {
        $conn = \Doctrine\DBAL\DriverManager::getConnection(['url' => $dbConnectionString]);
...
\React\EventLoop\Loop::addPeriodicTimer(10, function() use($executor, $dbConnectionString) {
    print "Going to get user's count...\n";
    $executor->submit(function () use($dbConnectionString) {
        $conn = \Doctrine\DBAL\DriverManager::getConnection(['url' => $dbConnectionString]);
        $sql = "SELECT count(*) as uc FROM users";
        $stmt = $conn->prepare($sql);
